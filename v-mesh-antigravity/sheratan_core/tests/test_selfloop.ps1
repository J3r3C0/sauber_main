...
for ($i = 1; $i -le 30; $i++) {
    ...
    try {
        ...
        $doneCount = ($selfloopJobs | Where-Object { $_.status -eq "completed" }).Count
        $totalCount = $selfloopJobs.Count
        
        Write-Host "`r   Progress: $doneCount/$totalCount jobs completed (checking... $i/30)" -NoNewline
        
        # Wenn alle 3 Iterationen completed sind, stoppen
        if ($doneCount -eq 3) {
...
try {
    ...
    foreach ($j in $selfloopJobs | Sort-Object created_at) {
        ...
        if ($status -eq "completed" -and $j.result) {
    ...
    Write-Host "      Done: $(($selfloopJobs | Where-Object {$_.status -eq 'completed'}).Count)"
