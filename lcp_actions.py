...
from .selfloop_prompt_builder import build_selfloop_prompt


class LCPActionInterpreter:
    """Interpretiert Worker-LCP-Resultate und erzeugt Folgejobs."""

    def __init__(self, bridge: WebRelayBridge, dispatch_fn: Optional[Any] = None) -> None:
        self.bridge = bridge
    ...
    def handle_job_result(self, job: models.Job) -> List[models.Job]:
        ...
        # Boss Directive 4.0: Support new_jobs in ANY successful LCP result
        elif res.get("new_jobs") and isinstance(res.get("new_jobs"), list):
            print(f"[lcp_actions] Found 'new_jobs' in '{action}' result, processing resonance...")
            created_jobs.extend(self._handle_followups(job, res.get("new_jobs")))
...
Create followup jobs to continue the analysis."""
                ...
                job_payload = {
                    ...
                    "provenance": {
                        "source": "core_auto_agent_plan",
                        "source_zone": "narrative"
                    }
                }
                ...
                # Dispatch
                if self.dispatch_fn:
                    self.dispatch_fn(analysis_job.id)
                else:
    ...
    def _handle_list_files(
        ...
        for path in files:
            ...
            payload = {
                "file": path,
                "provenance": {
                    "source": "core_list_files_followup",
                    "source_zone": "narrative"
                }
            }
    ...
    def _handle_followups(
        ...
        for spec in specs:
            ...
            # Ensure provenance for G0
            if "provenance" not in params:
                params["provenance"] = {
                    "source": "core_lcp_followup",
                    "source_zone": "narrative"
                }
            elif "source_zone" not in params["provenance"]:
                params["provenance"]["source_zone"] = "narrative"
            ...
            # Dispatch to worker queue (async, file-based) if auto_dispatch is True
            should_dispatch = spec.get("auto_dispatch", True)
            if should_dispatch:
                if self.dispatch_fn:
                    self.dispatch_fn(new_job.id)
                else:
                ...
                print(f"📦 Followup job created (pending manual dispatch): {new_job.id} (kind={task_kind}, name={job_name})")
        ...
        for line in section_d.splitlines():
            ...
            jc = models.JobCreate(
                payload={
                    ...
                    "provenance": {
                        "source": "core_selfloop_section_d",
                        "source_zone": "narrative"
                    }
                }
            )
    ...
    def _handle_selfloop_result(self, job: models.Job) -> List[models.Job]:
        ...
        if isinstance(result, dict):
            # Prefer "markdown" field from worker for Self-Loops, fallback to text/output
            result_text = result.get("markdown") or result.get("text") or result.get("output") or str(result)
        else:
        ...
        next_task_focus = parsed.get("D", "Continue iteration").strip()
        
        # Re-generate prompt for next iteration
        next_prompt = build_selfloop_prompt(
            goal=job.payload.get("goal"),
            core_data=job.payload.get("input_context", {}).get("core_data"),
            current_task=next_task_focus,
