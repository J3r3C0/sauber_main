@echo off
REM ============================================
REM Sheratan FULL System Start (Dry-Run + WebRelay)
REM ============================================

echo [SHERATAN] Starting FULL system with WebRelay Worker...
echo.
...
echo.
echo ============================================
echo [SHERATAN] FULL System is LIVE (DRY-RUN + WebRelay)
echo ============================================
echo.
echo Services running:
echo   1. Core API (port 8001)
echo   2. Journal Sync API (port 8100)
echo   3. Python Worker Loop (file operations)
echo   4. WebRelay Worker (LLM calls via ChatGPT/Claude)
echo.
echo Next steps:
echo 1. Send test jobs: python mobile_cli.py launch
echo 2. Watch logs in the 4 terminal windows
